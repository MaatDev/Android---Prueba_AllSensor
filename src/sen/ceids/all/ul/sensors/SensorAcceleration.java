...
public class SensorAcceleration extends MySensorFather{
	
	//Parámetros del sensor(m/s^2)
	private float lateral; //X
	private float longitudinal;  //Y
	private float vertical;  //Z
	
	public SensorAcceleration(Context context){
		
		super(context);
		
		super.sensorType = Sensor.TYPE_ACCELEROMETER;
		...
		super.mySensorListener = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				...
				if(isMySensorType(event.sensor.getType())){
					...
					Prueba_Sensores.tv_acceleration.setText(lateral + " / " + longitudinal + " / " + vertical);
